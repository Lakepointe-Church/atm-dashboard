...
export function SectionHeader({ title, sub, accent = colors.red, marginBottom = '16px', style }: {
  title: string
  sub?: string
  accent?: string
  ...
  return (
    <div style={{ marginBottom, ...style }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: sub ? '5px' : 0 }}>
        ...
        <div style={{ fontFamily: fonts.display, fontWeight: 600, fontSize: '20px', letterSpacing: '-0.01em', color: colors.ink, lineHeight: 1.1 }}>{title}</div>
      </div>
